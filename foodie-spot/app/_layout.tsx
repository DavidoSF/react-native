    ...
    <ThemeProvider value={colorScheme === 'dark' ? DarkTheme : DefaultTheme}>
      {!isOnline && (
        <View style={styles.offlineBanner}>
          <Ionicons name="cloud-offline-outline" size={16} color={theme.onBrand} />
      ...
      {isOnline && pendingCount > 0 && (
        <TouchableOpacity style={styles.syncBanner} onPress={syncNow} disabled={isSyncing}>
          <Ionicons name={isSyncing ? 'sync' : 'sync-outline'} size={16} color={theme.onBrand} />
          <Text style={styles.bannerText}>
...
const createStyles = (theme: typeof Colors.light) =>
  StyleSheet.create({
    ...
    bannerText: { color: theme.onBrand, fontSize: 14, fontWeight: '600' },
  });
