...
export default function NotificationScreen() {
  ...
  const {
    ...
    (notification) => {
      addTestResult(
        `✅ Notification reçue: ${notification.request.content.title}`,
      );
    },
    (data) => {
  ...
  const handleSchedule5Seconds = async () => {
    ...
    try {
      ...
      addTestResult(
        `✅ Notification Programmée pour ${date.toLocaleTimeString()}`,
      );
        ...
        {isSimulator && (
          <View style={styles.infoBox}>
            ...
            <Text style={styles.infoText}>
              Mode simulateur: Les notifications locales fonctionnent, mais les
              push tokens Expo nécessitent un appareil physique.
            </Text>
          </View>
        )}
      </ScrollView>
